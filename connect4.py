...
class Connect4:
    ...
    def iter_Ns(self, n):
        # horizontal
        for n_row in range(self.height):
            for start_column in range(self.width - n + 1):
                yield self.board[n_row][start_column:start_column + n]

        # vertical
        for n_column in range(self.width):
            for start_row in range(self.height - n + 1):
                yield [self.board[n_row][n_column] for n_row in range(start_row, start_row + n)]

        # diagonal
        for n_row in range(self.height - n + 1):
            for n_column in range(self.width - n + 1):
                yield [self.board[n_row + i][n_column + i] for i in range(n)]  # decreasing
                yield [self.board[n_row + i][self.width - 1 - n_column - i] for i in range(n)]  # increasing

    def iter_fours(self):
        return self.iter_Ns(4)
